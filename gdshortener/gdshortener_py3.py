...
class GDBaseShortener(object):
    ...
    def lookup(self, short_url):
        ...
        req = urllib.request.Request("{0}/forward.php".format(self.shortener_url), urllib.parse.urlencode(data), headers)
        f_desc = opener.open(req, timeout = self._timeout)
        response = json.loads(f_desc.read())
        if 'url' in response:
            # Success!
            return html.parser.HTMLParser().unescape(urllib.parse.unquote(response['url']))
        else:
            # Error
    ...
    def shorten(self, url, custom_url = None, log_stat = False):
        ...
        opener = urllib.request.build_opener()
        headers = { 'User-Agent' : self._user_agent }
        req = urllib.request.Request("{0}/create.php".format(self.shortener_url), urllib.parse.urlencode(data), headers)
        f_desc = opener.open(req, timeout = self._timeout)
        response = json.loads(f_desc.read())
